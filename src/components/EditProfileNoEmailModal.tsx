...
interface EditProfileNoEmailModalProps {
  isOpen: boolean;
  onClose: () => void;
...
type EditMode = 'username' | 'password';

const EditProfileNoEmailModal: React.FC<EditProfileNoEmailModalProps> = ({ isOpen, onClose, currentUser, onProfileUpdated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onProfileUpdated('Password updated successfully!');
      onClose();
    } catch (err: any) {
          ...
          >
            ...
            <div className="p-6 pb-0">
              <div className="flex bg-black/20 p-1 rounded-xl border border-white/5">
                {(['username', 'password'] as EditMode[]).map((tab) => (
                  <button
                    key={tab}
                    type="button"
                    onClick={() => {
...
export default EditProfileNoEmailModal;
