...
const RecentHIstory = ({
	...
	const getVerdictColors = (verdict?: string) => {
		...
		if (normalized === "likely fake")
			return {
				badge: "border-red-400/30 bg-red-400/10 text-red-500 dark:text-red-100",
				text: "text-red-500 dark:text-red-400",
			};
		if (normalized === "suspicious")
			return {
				badge:
					"border-yellow-400/30 bg-yellow-400/10 text-yellow-600 dark:text-yellow-100",
				text: "text-yellow-600 dark:text-yellow-400",
			};
		if (normalized === "likely real")
			return {
				badge:
					"border-green-400/30 bg-green-400/10 text-green-600 dark:text-green-100",
				text: "text-green-600 dark:text-green-400",
			};
		if (normalized === "satire")
			return {
				badge:
					"border-pink-400/30 bg-pink-400/10 text-pink-600 dark:text-pink-100",
				text: "text-pink-600 dark:text-pink-400",
			};
		if (normalized === "unreadable")
			return {
				badge:
					"border-slate-400/30 bg-slate-400/10 text-slate-600 dark:text-slate-100",
				text: "text-slate-600 dark:text-slate-400",
			};
		// default / anything else
		return {
			badge:
				"border-blue-400/30 bg-blue-400/10 text-blue-600 dark:text-blue-100",
		...
		>
			...
			{UserHistory.length === 0 && (
				<div className='flex flex-col items-center justify-center h-[80vh]'>
					<>
						<h1 className='text-lg font-semibold'>Nothing captured yet. </h1>
						<p className='text-muted-foreground'>
							Select anything on your screen and let{" "}
							<span className='font-semibold text-cyan-500'>RealityLens</span>{" "}
							do the rest.
						</p>
					</>
				</div>
